...
- 탐지 예외: `cam.js`(feature/babel-camera@298b19c 이후)가 `detectForVideo`를 try/catch로 감싸 직전 결과를 유지하고 `console.warn` 1회 → 11번은 그대로 진행. 추가로 21점 미만 랜드마크는 필터되고 프레임 정지 500ms 초과 시 `n: 0`으로 비워진다. 11번은 그와 별개로 `hands()`의 x·y가 유한값일 때만 손 입력으로 받는다(NaN 오염 방지).
