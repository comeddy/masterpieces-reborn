...
- **Mona Lisa's smile**: an image-reveal solution made the smile visible but was rejected as "too photographic"; a hard-edged high-density face patch was rejected as "alien to the rest" and for making the opening gather awkwardly. The current version has no face patch at all: dots sit on a single uniform grid at a higher overall density (up to about 18,000 dots, engine cap raised for this work only), so no region is denser than another. Dots snap to feature pixels (lip line, eyelids) so the smile survives sampling, and the gather is choreographed so body and background condense first and the face rises last
...
- In Birth of Venus (No. 02), press 📷 and your open hand becomes the west wind: sweep it to scatter the sea-foam that forms the goddess, hold it still and a soft breath parts the foam beneath it, close a fist and open it again for a gust, and a fast sweep leaves rose petals in its wake. Wind and gusts also work with the mouse (drag and click)
...
- In Impression, Sunrise (No. 05), press 📷 and stir the water with an open palm; close a fist and open it again and the sun bursts into rays
- In The Night Watch (No. 06), press 📷 and your raised hand becomes the lantern that lights the figures in the dark; close a fist and open it again for a gunpowder flash
...
- **모나리자의 미소**: 이미지 리빌 방식은 미소를 보이게 했지만 "너무 사실적"이라 반려되었고, 경계가 뚜렷한 얼굴 고밀도 패치는 "주변과 이질적"이며 시작 응집이 어색하다는 피드백을 받았습니다. 현재판은 얼굴 패치를 두지 않고 전 영역을 같은 간격의 균일 격자로 놓되 전체 밀도를 올렸습니다(최대 약 18,000점, 이 작품만 엔진 캡 상향). 어느 영역도 다른 곳보다 촘촘하지 않습니다. 점을 특징 픽셀(입술선·눈꺼풀)에 스냅해 미소가 샘플링에서 살아남게 하고, 몸·배경이 먼저 응결하고 얼굴이 마지막에 떠오르도록 응집을 안무합니다
...
- 02번 비너스의 탄생에서 📷를 누르면 펼친 손이 서풍이 됩니다. 손을 휘저으면 여신을 이루는 바다 거품이 흩날리고, 손을 가만히 두면 그 아래 거품이 숨결에 살짝 갈라집니다. 주먹을 쥐었다 펼치면 돌풍이 몰아치고, 손이 빠르게 지나간 자리에는 장미 꽃잎이 날립니다. 바람과 돌풍은 마우스로도 가능합니다(드래그·클릭)
...
- 05번 해돋이에서 📷를 누르고 펼친 손으로 수면을 휘저으면 물결이 일고, 주먹을 쥐었다 펼치면 태양이 광선을 뿜습니다
- 06번 야경에서 📷를 누르면 들어 올린 손이 등불이 되어 어둠 속 인물들을 비추고, 주먹을 쥐었다 펼치면 화약이 번쩍입니다
