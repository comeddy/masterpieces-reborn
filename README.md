...
- **Mona Lisa's smile**: an image-reveal solution made the smile visible but was rejected as "too photographic"; a hard-edged high-density face patch was rejected as "alien to the rest" and for making the opening gather awkwardly. The current version has no face patch at all: one uniform grid at a higher overall density (about 18,000 dots, engine cap raised for this work only), so no region is denser than another. Dots snap to feature pixels (lip line, eyelids) so the smile survives sampling, and the gather is choreographed so body and background condense first and the face rises last
...
- **모나리자의 미소**: 이미지 리빌 방식은 미소를 보이게 했지만 "너무 사실적"이라 반려되었고, 경계가 뚜렷한 얼굴 고밀도 패치는 "주변과 이질적"이며 시작 응집이 어색하다는 피드백을 받았습니다. 현재판은 얼굴 패치를 두지 않고 전 영역을 같은 간격의 균일 격자로 놓되 전체 밀도를 올렸습니다(약 18,000점, 이 작품만 엔진 캡 상향). 어느 영역도 다른 곳보다 촘촘하지 않습니다. 점을 특징 픽셀(입술선·눈꺼풀)에 스냅해 미소가 샘플링에서 살아남게 하고, 몸·배경이 먼저 응결하고 얼굴이 마지막에 떠오르도록 응집을 안무합니다
