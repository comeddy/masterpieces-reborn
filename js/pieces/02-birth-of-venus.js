...
export const HAND = {
  ...
  breathForce: 80,     // 숨결 방사 가속(px/s², 손 중심에서 최대·가장자리 0) — 스프링(2.3~4.4)과 평형을 이뤄 손 아래가 20~35px 살짝 갈라진다
  petalSpeedMin: 320,  // 이 속도(px/s) 이상으로 손이 지나가면 꽃잎이 흩날린다
  petalCool: 0.16,     // 손 꽃잎 방출 간격(s)
  petalMax: 90,        // 화면 위 꽃잎 총량 상한(비용 상한) — 손 방출만 제한, 가장자리 꽃잎은 기존대로
};
...
// 반환 null = 이번 프레임 방출 없음. petalCount가 petalMax 이상이면 방출하지 않는다.
export function petalEmit(state, ptr, dt, reduced, petalCount) {
  state.cool = Math.max(0, state.cool - dt);
  if (!isHand(ptr) || !ptr.down) return null;
  const speed = ptr.hand.speed;
  if (!Number.isFinite(speed) || speed < HAND.petalSpeedMin) return null;
  if (state.cool > 0 || petalCount >= HAND.petalMax) return null;
...
// 저항(drag)으로 느려지고 가볍게 내려앉아(g) 바람·팔랑임에 실린다.
...
function updatePetals(dt, wind) {
  for (let i = petals.length - 1; i >= 0; i--) {
    ...
    p.rot += p.vr * dt;
    if (p.x < -160 || p.x > W + 160 || p.y > H + 80 || p.y < -120) petals.splice(i, 1);
  }
}
...
export default {
  init(opts) {
    ...
    petals = []; petalTimer = reduced ? 6 : 3.5; handPetal = { cool: 0 };
  ...
  tick(dt, ptr) {
    ...
    const e = ok ? petalEmit(handPetal, ptr, dt, reduced, petals.length) : null;
  ...
  dispose() {
    field = null; petals = []; bgGrad = null; ctx = null; handPetal = { cool: 0 };
  },
};
