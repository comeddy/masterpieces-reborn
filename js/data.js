...
export const WORKS = [
  ...
  },
  {
    ...
    note: "보티첼리의 비너스가 바다 거품으로 빚어진다. 서풍의 신이 그랬듯 당신의 드래그가 바람이 되어 여신을 흩날리고, 장미 꽃잎이 화면을 가로지른다. 바람이 멎으면 거품은 다시 여신의 형상으로 피어오른다. 카메라 앞에서는 펼친 손이 서풍이 된다. 손을 가만히 두면 그 아래 거품이 숨결에 갈라지고, 주먹을 쥐었다 펼치면 돌풍이 몰아치며, 빠르게 지나간 자리에는 장미 꽃잎이 날린다.",
    hint: "📷를 켜고 손을 펼쳐 바람을 일으키세요 · 가만히 두면 숨결 · 주먹을 쥐었다 펼치면 돌풍 · 마우스는 드래그로 바람을 일으키고 클릭으로 돌풍",
    cam: true, handPointer: true, camHands: 1,
  ...
  },
  {
    ...
    note: "레오나르도의 스푸마토 — '연기처럼 사라지는' 기법 그대로, 모나리자가 안개 입자로 흩어지고 응결된다. 휘저으면 초상은 갈색 연무가 되고, 안개가 걷히며 얼굴이 돌아올 때 그 얼굴은 언제나 가장 늦게 떠오른다. 카메라 앞에서 펼친 손을 움직이면 그 손이 안개를 휘젓고, 주먹을 쥐었다 펼치면 초상이 연무로 흩어진다.",
    hint: "📷를 켜고 손을 펼쳐 안개를 휘저으세요 · 주먹을 쥐었다 펼치면 흩어집니다 · 얼굴을 기다리세요 · 마우스는 드래그로 휘젓고 클릭으로 흩뜨리기",
    cam: true, handPointer: true, camHands: 1,
  ...
  // ---- Ⅱ관 · 빛의 방 — 빛과 대기의 재해석 ----
  {
    ...
    note: "인상주의라는 이름을 낳은 르아브르의 새벽이 실시간으로 다시 밝아온다. 밝은 노랑 태양이 고동치고, 부서진 반사광이 수면 위에서 명멸하며, 조각배는 안개 속을 느리게 지난다. 태양을 클릭하면 눈부신 빛이 사방으로 뻗어나간다. 카메라 앞에서 펼친 손이 수면을 휘젓고, 주먹을 쥐었다 펼치면 태양이 광선을 뿜는다.",
    hint: "📷를 켜고 손을 펼쳐 수면을 휘저으세요 · 주먹을 쥐었다 펼치면 태양이 눈부시게 빛납니다 · 마우스는 드래그로 물결, 클릭으로 태양",
    cam: true, handPointer: true, camHands: 1,
  ...
  },
  {
    ...
    note: "렘브란트의 대작이 온전한 어둠에 잠겨 있다. 당신의 커서가 등불이 되어 어둠을 밝히면, 빛이 닿는 자리에서만 대원들의 얼굴이 떠오른다. 등불이 지나간 자리엔 잔광이 머물다 스러진다 — 빛과 어둠의 화가에게 바치는 오마주. 카메라 앞에서는 당신의 손이 등불이 되어 어둠 속 인물들을 비추고, 주먹을 쥐었다 펼치면 화약이 번쩍인다.",
    hint: "📷를 켜고 손을 들어 어둠을 비추세요 · 주먹을 쥐었다 펼치면 화약 섬광 · 마우스는 커서로 비추고 클릭으로 섬광",
    cam: true, handPointer: true, camHands: 1,
  ...
  },
  {
    ...
    note: "반 고흐가 아를의 노란 집에서 그린 해바라기들이 임파스토 붓질로 피어난다. 꽃들은 저마다의 속도로 피고 시들기를 반복하고 — 시든 꽃을 클릭하면 다시 만개하며 꽃가루를 터뜨린다. 생명과 소멸을 오가는 노랑의 순환. 카메라 앞에서 손을 흔들면 붓바람이 꽃대를 눕히고, 주먹을 쥐었다 펼치면 손 아래 꽃이 활짝 핀다.",
    hint: "📷를 켜고 손을 좌우로 흔들어 붓바람을 일으키세요 · 주먹을 쥐었다 펼치면 가까운 꽃이 만개 · 마우스는 드래그로 붓바람, 클릭으로 개화",
    cam: true, handPointer: true, camHands: 1,
  ...
  },
  {
    ...
    note: "터너의 금빛 폭풍 속에서 증기기관차가 다가온다. 안개와 비와 증기가 한 덩어리의 유동장이 되어 소용돌이치고, 기차는 소실점에서부터 화실의 불꽃을 키우며 끝없이 달려온다. 산업 시대의 속도를 화폭에 담아낸 가장 이른 그림 중 하나 — 그 속도 그대로. 마이크에 숨을 불어 넣으면 기차가 숨을 쉬듯 증기를 뿜고 안개가 밀려나며, 외침이나 박수에 기적이 터진다.",
    hint: "🎤를 켜고 숨을 불면 굴뚝이 증기를 뿜고 안개가 밀려납니다 · 외치거나 박수 치면 기적 · 마우스는 드래그로 안개, 클릭으로 기적",
    mic: true,
  ...
  },
  {
    ...
    note: "브뤼헐의 바벨탑은 완성되지 못할 것을 알면서도 쌓아 올려진다. 벽돌은 저절로 한 장씩 놓이고, 당신은 클릭으로 건설을 거들 수도, 길게 눌러 무너뜨릴 수도 있다. 무너진 자리에서도 건설은 계속된다 — 인간의 끝없는 오만과 열망. 카메라 앞에 한 손을 들면 탑이 자라고, 두 손을 잠시 들고 있으면 무너진다.",
    hint: "📷를 켜고 한 손을 들면 쌓고 두 손을 잠시 들면 무너집니다 · 마우스는 클릭으로 쌓고 길게 누르면 무너집니다",
    cam: true,
  ...
  },
  {
    ...
    note: "피카소는 하나의 얼굴을 여러 시점에서 동시에 보았다. 조각난 면들이 정면과 옆모습을 한 화면에 겹쳐 놓고, 클릭할 때마다 얼굴은 전혀 새로운 구성으로 다시 조립된다 — 매 순간이 단 한 번뿐인 초상. 특정 원작의 복제가 아닌, 큐비즘이라는 발명 자체에 바치는 오마주. 카메라 앞에서 손을 크게 흔들면 초상은 그 몸짓에 응답해 새로 태어난다.",
  ...
  // ---- Ⅳ관 · 한국의 방 — 조선 회화, 신규 기법 ----
  {
    ...
    note: "비 갠 인왕산의 물기 어린 공기가 되살아난다. 안개가 산허리를 감싸며 흘렀다 걷히기를 반복하고, 화면을 누르면 먹 한 방울이 화선지에 스며 번져나간다. 진경산수의 바위 절벽은 먹빛이 깊을수록 단단해진다 — 겸재가 그린 비 갠 아침 그대로. 마이크에 숨을 불면 비 갠 산의 안개가 걷히고 조용해지면 다시 차오르며, 외침이나 박수 소리에 먹 한 방울이 떨어진다.",
    hint: "🎤를 켜고 숨을 불면 산허리 안개가 걷힙니다 · 외치거나 박수 치면 먹 한 방울 · 마우스는 드래그로 안개, 누르면 먹",
    mic: true,
  ...
  },
  {
    ...
    note: "단원의 씨름판이 실제로 벌어진다. 구경꾼 하나하나가 저마다의 리듬으로 들썩이고 부채를 부치다가, 씨름꾼이 기술을 거는 순간 환호가 물결처럼 번져나간다. 그 소란 속에서도 엿장수만은 무심히 제 갈 길을 간다 — 단원이 숨겨둔 웃음 그대로. 마이크에 환호를 보내면 소리 크기만큼 구경꾼이 들썩이고, 외침이나 박수에 씨름꾼이 기술을 건다.",
    hint: "🎤를 켜고 환호하세요 — 소리가 클수록 구경꾼이 들썩입니다 · 외치거나 박수 치면 기술 · 마우스는 클릭으로 기술, 드래그로 파도응원",
    mic: true,
  ...
  },
  {
    ...
    note: "'달빛 침침한 삼경, 두 사람 마음은 두 사람만 안다(月沈沈夜三更 兩人心事兩人知).' 혜원이 화폭 한켠에 적어둔 그 밤이 흐른다. 초승달이 천천히 차고 기울며 밤의 깊이가 변하고, 당신의 커서가 구름이 되어 달을 가리면 — 초롱불 하나만 남은 어둠 속에서 밀회는 조금 더 깊어진다. 마이크에 바람 소리를 내면 구름이 달로 흘러가 밤이 깊어지고, 외침이나 박수 소리에 초롱불이 깜빡인다.",
    hint: "🎤를 켜고 \"후—\" 바람 소리를 내면 구름이 달을 가립니다 · 외치거나 박수 치면 초롱불이 깜빡 · 마우스는 커서로 구름, 클릭으로 초롱불",
    mic: true,
  ...
  },
  {
    ...
    note: "안평대군이 꿈에서 본 복사꽃 이상향을 안견이 사흘 만에 그렸다. 두루마리를 펼치듯 화면을 끌면 왼쪽의 현실 세계에서 험준한 기암절벽을 지나 오른쪽 도원경으로 여행이 이어진다. 가만히 두면 꿈이 스스로 흘러간다 — 도원에 이르면 복사꽃잎이 바람에 날린다. 카메라 앞에서 펼친 손을 옆으로 움직이면 두루마리가 손을 따라 펴지고, 손을 내리면 꿈이 다시 제 속도로 흐른다.",
    hint: "📷를 켜고 펼친 손을 옆으로 움직여 두루마리를 펼치세요 · 손을 내리면 꿈이 다시 스스로 흐릅니다 · 마우스는 드래그로 펼치고 클릭으로 꽃잎",
    cam: true, handPointer: true, camHands: 1,
