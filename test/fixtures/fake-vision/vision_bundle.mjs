...
export class HandLandmarker {
  ...
  detectForVideo() {
    ...
    const mk = (sx, y, pinch, open, fist) => {
      ...
      if (open) {                                          // 활짝: 다섯 손끝을 손목에서 ≈1.9×손바닥 거리로
        const ty = y - 0.11;
        [[4, -0.06], [8, -0.03], [12, 0], [16, 0.03], [20, 0.06]].forEach(([i, dx]) => { lm[i] = { x: x + dx, y: ty, z: 0 }; });
      } else if (fist) {                                   // 주먹: 손끝을 손바닥 안으로(≈0.8×)
        for (const i of [4, 8, 12, 16, 20]) lm[i] = { x, y: y + 0.02, z: 0 };
      }
      return lm;
    };
    const landmarks = [];
    if (s.n >= 1) landmarks.push(mk(s.x, s.y, !!s.pinch, !!s.open, !!s.fist));
    if (s.n >= 2) landmarks.push(mk(Math.min(1, s.x + 0.2), s.y, false, !!s.open, !!s.fist));
    return { landmarks };
  }
  close() {}
}
